...
- [1.2 Données utilisées](#12-données-utilisées)
- [1.3 Hypothèse de modélisation](#13-hypothèse-de-modélisation)
- [1.4 Placement des dépôts](#14-placement-des-dépôts)
- [1.5 Formalisation et méthode de résolution](#15-formalisation-et-méthode-de-résolution)
- [1.6 Limites du modèle](#16-limites-du-modèle)

2. Résultats et Scénarios
- [2.1 Scénario 0](#21-scénario-0)
- [2.2 Scénario 1](#22-scénario-1)
- [2.3 Scénario 2](#23-scénario-2)
- [2.4 Scénario 3](#24-scénario-3)
...
- On sélectionne un milieu d'arête au hasard parmi les arêtes dont les nœuds font partie des 25% des nœuds au plus fort degré. Ce milieu d'arête est notre premier centroïde.
- Pour chaque autre milieu d'arête restant, on calcule sa distance au centroïde le plus proche déjà choisi. On élève cette distance au carré ce qui nous donne un poids. En fonction de ces poids on trouve le centroïde suivant à ajouter. On répète l'opération autant de fois qu'on veut de dépôts.
- Enfin chaque centroïde trouve le nœud le plus proche et sélectionne ce nœud comme dépôt.
...
- Sélection des dépôts : le premier milieu est choisi au hasard, donc il n'est pas nécessairement optimal. Les dépôts sont forcément positionnés à des carrefours, ce qui n'est pas réaliste.
...
- Équilibrage des degrés : on compare les degrés entrant et sortant de chaque nœud. On liste les nœuds en excès et les nœuds en déficit. Pour chaque nœud excédentaire on regarde le nœud déficitaire le plus proche géographiquement. Or le plus proche géographique n'est pas forcément le plus proche dans le graphe.
...
Ce scénario constitue la référence pour le déneigement total de la ville. Toutes les rues reçoivent la même priorité. Les dépôts sont placés automatiquement (voir [1.4 Placement des dépôts](#14-placement-des-dépôts)).

Argumentaire : le déneigement uniforme est l'approche réglementaire de base. Il sert de référence pour les autres scénarios.

Résultats :

| Nombre de dépôts | Nombre de déneigeuses par dépôt | Temps nécessaire | Prix hors coût dépôt | Prix avec coût dépôt |
|---|---|---|---|---|
...
| Arrondissement | Dépôts | Nombre de dépôts x nombre de déneigeuses | Temps nécessaire | Prix hors coût dépôt | Prix avec coût dépôt |
|---|---|---|---|---|---|
| Verdun | 1 | 1×2 | 8.66h | 1 179,63 $ | 1 185,32 $ |
| Outremont | 1 | 1×1 | 8.88h | 607,64 $ | 613,33 $ |
| Anjou | 1 | 1×4 | 9.61h | 2 391,05 $ | 2 396,74 $ |
| RDP | 2 | 2×6 | 9.72h | 7 149,25 $ | 7 159,78 $ |
...
|---|---|---|---|---|---|---|---|---|
| Verdun | 1 | 1×2 | 8.66h | 1 179,39 $ | 1 185,08 $ | 6.93h | 8.61h | −1.69h (−20%) |
| Outremont | 1 | 1×1 | 8.88h | 607,64 $ | 613,33 $ | 8.34h | 8.63h | −0.29h (−3%) |
| Anjou | 1 | 1×4 | 9.76h | 2 388,66 $ | 2 394,34 $ | 9.40h | 9.43h | −0.02h (−0%) |
| RDP | 2 | 1×8 / 1×6 | 9.44h | 8 183,82 $ | 8 194,35 $ | 9.30h | 9.53h | −0.24h (−2%) |
...
|---|---|---|---|---|---|---|---|---|
| Verdun | 1 | 1×2 | 8.66h | 1 179,63 $ | 1 185,32 $ | 7.88h | 8.38h | −0.50h (−6%) |
| Outremont | 1 | 1×1 | 8.88h | 607,64 $ | 613,33 $ | 8.14h | 8.63h | −0.49h (−6%) |
| Anjou | 1 | 1×4 | 9.61h | 2 391,05 $ | 2 396,74 $ | 8.69h | 8.96h | −0.27h (−3%) |
| RDP | 2 | 1×8 / 1×7 | 9.65h | 8 691,71 $ | 8 702,24 $ | 7.33h | 8.43h | −1.10h (−13%) |
...
Modèle de quantification du gain économique local. Pour mesurer l'impact réel du scénario 2, au-delà des simples coûts opérationnels de transport (qui représentent néanmoins 200M de dollars en budget municipal annuel), prenons l'exemple de l'arrondissement de Rivière-des-Prairies (106 000 habitants) : notre modèle permet à 52 000 habitants d'accéder aux supermarchés et aux pharmacies 4 heures plus tôt qu'avec le scénario de base. D'après l'Institut de la statistique du Québec (ISQ), les dépenses moyennes en commerce s'élèvent à environ 45 $ par jour par habitant. En posant l'hypothèse prudente que seulement 15% de ces 52 000 habitants ont besoin d'approvisionnement ce jour-là, on peut calculer :

> Gain RDP : 52 000 hab. × 15% × 45 $ = 351 000 $ préservés.
...
|---|---|---|---|---|---|---|---|---|
| Verdun | 1 | 1×2 | 8.66h | 1 175,53 $ | 1 181,22 $ | 5.61h | 6.01h | −0.40h (−7%) |
| Outremont | 1 | 1×1 | 8.88h | 607,64 $ | 613,33 $ | 6.64h | 8.07h | −1.43h (−18%) |
| Anjou | 1 | 1×4 | 9.79h | 2 393,13 $ | 2 398,82 $ | 6.91h | 7.81h | −0.91h (−12%) |
| RDP | 2 | 1×6 / 1×5 | 9.98h | 6 630,09 $ | 6 640,62 $ | 5.82h | 7.80h | −1.97h (−25%) |
